...
class MainWidget(RelativeLayout):
   ...
   SPEED = .8
   ACCELERATION = SPEED
   INCREASE = 0.002 # augmenter la vitesse au fur et à mesure, mettre une très petite valeur
   current_offset_y = 0
   current_y_loop = 0
   ...
   def init_audio(self):
       ...
       self.sound_gameover_impact = SoundLoader.load('audio/videogame_death.wav')
   ...
   def generate_tiles_coordinates(self):
       ...
       for i in range(len(self.tiles_coordinates), self.NB_TILES):
           ...
           # 1 -> droite
           # 2 -> gauche
           start_index = -int(self.V_NB_LINES / 2) + 0  # 1 si on ne veut pas sortir de la route
           end_index = start_index + self.V_NB_LINES - 0  # 2 si ne veut pas sortir de la route
   ...
   def update(self, dt):
       ...
       if not self.state_game_over and self.state_game_has_started:
           speed_y = self.ACCELERATION * self.height / 100
           ...
           while self.current_offset_y >= spacing_y:
               ...
               self.ACCELERATION += self.INCREASE
               self.score_txt = "SCORE: " + str(self.current_y_loop)
               self.generate_tiles_coordinates()
   ...
   def on_menu_button_pressed(self):
       ...
       if self.state_game_over:
           ...
           self.ACCELERATION = self.SPEED  # remettre l'accélération à 0 si on recommence
       else:
